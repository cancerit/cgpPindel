	...
	$sam_obj->fast_pileup("$chr:$g_r_start-$g_r_end", sub {
			...
			if($pos >= $g_r_start && $pos <= $g_r_end) {
				...
				foreach $pu (@{$pileup}) {
					...
					next if($g_e_type eq 'DI' && $pu->alignment->cigar_str !~ $g_match);	
					$abs = abs $value;
				...
				#	warn "change len: $abs $g_chg_len, ".$record->ref_seq;
					...
					if($abs == $g_chg_len || $abs == $g_min_chg_len) {
						#$strand_index = $pu->alignment->strand == 1 ? 1 : 0;
						$strand_index = $pu->alignment->reversed == 1 ? 0 : 1;
 					    #$rn = $pu->alignment->query->name;
 					    $rn = $pu->alignment->qname;
