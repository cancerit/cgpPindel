...
sub flag_010 {
  my ($MATCH,$CHROM,$POS,$FAIL,$PASS,$RECORD,$VCF) = @_;
  if(exists $vcf_flagging_unmatched_normals_tabix->{$CHROM}
  && exists $vcf_flagging_unmatched_normals_tabix->{$CHROM}->{$POS}
  && defined $vcf_flagging_unmatched_normals_tabix->{$CHROM}->{$POS}) {
    return $FAIL;
  }
  return $PASS
}
